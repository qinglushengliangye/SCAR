#!/bin/bash
# ============================================================
# FewRel ablation 3-seed 重复验证 launcher
# 5 个 ablation 模块 × 3 seed = 15 实验 (已有 5, 新增 10)
#   CCA  × {no_grad_iso, no_zscore}                          × 3 seed
#   ISCL × {no_cross_attn, no_global_align, no_supcon_warmup} × 3 seed
#
# 用法: bash launch_rerun_batch_fewrel.sh <batch_id>
...
#   batch 4: ISCL × seed 99887766  (3 jobs → GPU 0, 1, 2)
# ============================================================
...
if [ -z "$1" ]; then
    ...
    echo ""
    echo "  batch 1: FewRel CCA  ablations × seed 11222333  (2 jobs)"
    echo "  batch 2: FewRel CCA  ablations × seed 55667788  (2 jobs)"
    echo "  batch 3: FewRel ISCL ablations × seed 11223344  (3 jobs)"
    echo "  batch 4: FewRel ISCL ablations × seed 99887766  (3 jobs)"
    exit 1
fi

BATCH=$1
CFG_DIR=/root/GLiREL/configs/rerun_ablation
BASE_LOG=/root/autodl-tmp/logs-m=15/logs_ablation_rerun_fewrel

case $BATCH in
    1)
        SEED=11222333
        CONFIGS=("fewrel_cca_no_grad_iso_seed${SEED}" "fewrel_cca_no_zscore_seed${SEED}")
        ;;
    2)
        SEED=55667788
        CONFIGS=("fewrel_cca_no_grad_iso_seed${SEED}" "fewrel_cca_no_zscore_seed${SEED}")
    ...
    3)
        SEED=11223344
        CONFIGS=("fewrel_iscl_no_cross_attn_seed${SEED}" "fewrel_iscl_no_global_align_seed${SEED}" "fewrel_iscl_no_supcon_warmup_seed${SEED}")
        ;;
    4)
        SEED=99887766
        CONFIGS=("fewrel_iscl_no_cross_attn_seed${SEED}" "fewrel_iscl_no_global_align_seed${SEED}" "fewrel_iscl_no_supcon_warmup_seed${SEED}")
...
echo "============================================================"
echo " FewRel Batch $BATCH  (seed=$SEED, ${#CONFIGS[@]} jobs)"
echo " Configs:"
...
echo ""
echo "============================================================"
echo "FewRel Batch $BATCH launched.  PIDs: ${PIDS[*]}"
echo ""
echo "Monitor:"
