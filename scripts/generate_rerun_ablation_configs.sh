#!/bin/bash
# ============================================================
# 生成 12 个新的 Wiki-ZSL ablation config 文件
# 每个 ablation 在 2 个新 seed 上重复（除原 seed 外）
#
# CCA 3 ablations:  新 seeds = 11222333, 55667788  (原 seed: 11223344)
# ISCL 3 ablations: 新 seeds = 11223344, 33445566  (原 seed: 20260421)
# ============================================================
set -e
...
ISCL_SEEDS=(11223344 33445566)
ISCL_ORIG_SEED=20260421

count=0
for abl in "${CCA_ABLATIONS[@]}"; do
    src=$SRC_DIR/config_ablation_cca_${abl}.yaml
    for new_seed in "${CCA_SEEDS[@]}"; do
        dst=$OUT_DIR/cca_${abl}_seed${new_seed}.yaml
...
for abl in "${ISCL_ABLATIONS[@]}"; do
    src=$SRC_DIR/config_ablation_iscl_${abl}.yaml
    for new_seed in "${ISCL_SEEDS[@]}"; do
        dst=$OUT_DIR/iscl_${abl}_seed${new_seed}.yaml
...
echo ""
echo "Generated $count config files under $OUT_DIR"
