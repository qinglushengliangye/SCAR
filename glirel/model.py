...
from glirel.modules.dual_encoder_retriever import DualEncoderRetriever
...
class GLiREL(InstructBase, PyTorchModelHubMixin):
    def __init__(self, config):
        ...
        if getattr(config, 'cascade_retrieval', False):
            ...
            fusion_alpha_max       = getattr(config, 'fusion_alpha_max', 1.0)
            warmup_steps           = getattr(config, 'cascade_warmup_steps', 2000)
            retrieval_loss_weight  = getattr(config, 'retrieval_loss_weight', 0.1)
            self.dual_encoder_retriever = DualEncoderRetriever(
                hidden_size=config.hidden_size,
                ...
                dropout=config.dropout,
                zscore_enabled=getattr(config, 'zscore_enabled', True),
                gradient_isolation=getattr(config, 'gradient_isolation', True),
            )
            logger.info(
                f"[CascadeAugment] DualEncoderAugmentor enabled | "
                f"retrieval_dim={retrieval_dim}, "
                f"fusion_alpha_max={fusion_alpha_max}, "
                f"warmup_steps={warmup_steps}, "
                f"retrieval_loss_weight={retrieval_loss_weight}"
            )
        # ─────────────────────────────────────────────────────────────────
        ...
        if getattr(config, 'supcon_enabled', False):
            self.supcon_module = SupervisedContrastiveModule(
                ...
                dropout=config.dropout,
                global_alignment=getattr(config, 'supcon_global_alignment', True),
                cross_attention=getattr(config, 'supcon_cross_attention', True),
            )
            logger.info(
                f"[SupCon] SupervisedContrastiveModule enabled | "
                f"proj_dim={getattr(config, 'supcon_proj_dim', 128)}, "
                f"temperature={getattr(config, 'supcon_temperature', 0.1)}, "
                f"hard_neg_beta={getattr(config, 'supcon_hard_neg_beta', 1.0)}, "
    ...
    def compute_score(self, x):
        ...
        if hasattr(self, 'dual_encoder_retriever'):
            ...
            aug_out = self.dual_encoder_retriever(
                fine_scores=scores,
                rel_rep=rel_rep,
            ...
            if self.training:
                ...
                if step_val % 500 == 1:
                    logger.info(
                        f"[CascadeAugment] step={step_val} "
                        f"fusion_alpha={aug_out['fusion_alpha']:.4f} "
                        f"retrieval_loss={retrieval_loss.item():.4f}"
                    )
        # ─────────────────────────────────────────────────────────────────
        ...
        if hasattr(self, 'supcon_module'):
            ...
            sup_out = self.supcon_module(
                rel_rep=rel_rep,
                rel_type_rep=rel_type_rep,
                rel_type_mask=rel_type_mask,
                rel_labels=_rel_labels_sup,
                class_to_ids_batch=_class_to_ids,
            )
            supcon_loss = sup_out['supcon_loss']
        # ─────────────────────────────────────────────────────────────────

        return scores, num_classes, rel_type_mask, coref_scores, retrieval_loss, supcon_loss  # ([B, num_pairs, num_classes]), num_classes, ([B, num_classes]), ([B, num_pairs, 1]), scalar, scalar
    ...
    def forward(self, x):
        # compute span representation
        scores, num_classes, rel_type_mask, coref_scores, retrieval_loss, supcon_loss = self.compute_score(x)
        ...
        output = {
            ...
            "retrieval_loss": retrieval_loss if hasattr(self, 'dual_encoder_retriever') else None,
            "supcon_loss": supcon_loss if hasattr(self, 'supcon_module') else None,
        }
    ...
    def predict(self, x, flat_ner=False, threshold: list | float = 0.5, ner=None, return_loss: bool = False):
        ...
        if return_loss:
            ...
            local_scores, num_classes, rel_type_mask, coref_scores, _retrieval_loss, _supcon_loss = self.compute_score(x)
        ...
        for thresh in threshold:
            ...
            # Fix for PyTorch 2.1 compatibility: flatten extra dims before all()
            valid_entity_mask = (entity_pairs != -1).view(entity_pairs.shape[0], -1).all(dim=1)
